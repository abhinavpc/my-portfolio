...
import { Plus, X, Trash2, Loader2, UploadCloud, ChevronDown } from 'lucide-react';
...
// --- Default Data ---
// Updated to reflect the "Still life, Portrait, Religious" description
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const artworksRef = collection(db, 'artifacts', appId, 'users', user.uid, 'artworks');
  ...
  const handleAddArtwork = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'artworks'), {
        title: newTitle,
  ...
  const handleDelete = async (e, itemId) => {
    ...
    if (String(itemId).startsWith('demo-')) {
       alert("Upload your own art to manage the collection.");
       return;
    }
    if (confirm("Remove this piece from the collection?")) {
      try { await deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'artworks', itemId)); } 
  ...
  return (
    <div className="min-h-screen bg-[#F5F2EF] text-[#2A2A2A] font-serif selection:bg-[#963F3F] selection:text-white">
      ...
      <main id="gallery" className="max-w-[1600px] mx-auto px-6 py-24 min-h-[50vh]">
            ...
            {artworks.map((art, index) => (
              ...
              >
                ...
                <div className="relative overflow-hidden bg-[#EBE8E4]">
                  ...
                  {!String(art.id).startsWith('demo-') && (
                    <button 
                      onClick={(e) => handleDelete(e, art.id)}
                      className="absolute top-4 right-4 text-white/50 hover:text-[#963F3F] transition-colors"
                    >
                      <Trash2 size={16} />
                    </button>
                  )}
        ...
        {!loading && artworks.length === 0 && (
          <div className="text-center py-32">
            <p className="font-cormorant text-2xl text-[#888] italic">The collection is quiet.</p>
          </div>
        )}
      </main>
      ...
      <section id="about" className="py-32 px-6 bg-[#EBE8E4] mt-20">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-16">
           <div className="w-full md:w-1/2 aspect-[3/4] bg-[#D1CEC7] relative overflow-hidden">
             {/* Placeholder for Artist Portrait - using a texture or muted image */}
      ...
      {/* --- Footer --- */}
      <footer id="contact" className="bg-[#1A1A1A] text-[#888] py-24 px-6">
      ...
      <button 
        ...
        className="fixed bottom-8 right-8 z-40 bg-[#1A1A1A] text-white p-4 rounded-full shadow-2xl hover:bg-[#963F3F] transition-colors duration-300"
        title="Add Artwork"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#2A2A2A]/40 backdrop-blur-sm p-4">
          <div className="bg-[#F5F2EF] p-10 max-w-lg w-full shadow-2xl relative animate-fade-in">
            <button 
              ...
              className="absolute top-6 right-6 text-[#888] hover:text-[#2A2A2A]"
            >
              <X size={20} />
            </button>
            ...
            <form onSubmit={handleAddArtwork} className="space-y-8">
              <div className="group">
                <label className="block text-[10px] uppercase tracking-wider text-[#888] mb-2 font-sans-clean group-focus-within:text-[#963F3F]">Title</label>
                <input 
                  type="text" 
                  value={newTitle}
                  onChange={(e) => setNewTitle(e.target.value)}
                  className="w-full bg-transparent border-b border-[#D1CEC7] py-2 font-cormorant text-2xl text-[#2A2A2A] focus:outline-none focus:border-[#963F3F] transition-colors placeholder:text-[#D1CEC7]"
              ...
              <div className="group">
                <label className="block text-[10px] uppercase tracking-wider text-[#888] mb-2 font-sans-clean group-focus-within:text-[#963F3F]">Medium</label>
                <input 
                  type="text" 
                  value={newMedium}
              ...
              <div className="flex gap-6 pt-4">
                <button 
                  ...
                  className={`text-xs uppercase tracking-widest pb-1 border-b transition-all ${uploadMode === 'url' ? 'border-[#963F3F] text-[#2A2A2A]' : 'border-transparent text-[#888] hover:text-[#2A2A2A]'}`}
                >
                  Link URL
                </button>
                <button 
                  ...
                  className={`text-xs uppercase tracking-widest pb-1 border-b transition-all ${uploadMode === 'file' ? 'border-[#963F3F] text-[#2A2A2A]' : 'border-transparent text-[#888] hover:text-[#2A2A2A]'}`}
                >
                  Upload File
                </button>
              </div>
              ...
              {uploadMode === 'url' ? (
                <input 
                  ...
                  className="w-full bg-transparent border-b border-[#D1CEC7] py-2 font-cormorant text-lg text-[#2A2A2A] focus:outline-none focus:border-[#963F3F] transition-colors"
                  placeholder="https://..."
              ...
              <button 
                ...
                className="w-full bg-[#1A1A1A] text-white py-4 mt-6 uppercase tracking-[0.2em] text-[10px] hover:bg-[#963F3F] transition-colors disabled:opacity-50"
              >
